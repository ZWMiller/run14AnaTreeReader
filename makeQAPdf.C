...
void makeQAPdf(const char* FileName="test")
{
	...
	sprintf(tlName, "RUN 14 Au+Au 200 GeV");
	...
	sprintf(tlName,"eID: -0.8 < n#sigma_{e TPC} < 2.5, #left|gDCA#right| < 1 cm");
	tl.DrawLatex(0.1, 0.6,tlName);
	sprintf(tlName,"n#phi > 1, n#eta > 1, pT > 0.2, #left|#eta#right| < 1");
	tl.DrawLatex(0.1, 0.53,tlName);
	sprintf(tlName,"Event: #left|V_{z}#right| < 30 cm, #left|V_{z}-V_{z-VPD}#right| < 3 cm");
	tl.DrawLatex(0.1,0.46,tlName);
	sprintf(tlName,"TQ: nHitsFit > 20, nHits#frac{dE}{dx} > 15, nHitFit/Max > 0.52");
